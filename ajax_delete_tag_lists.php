...
try {
    ...
    // Prepare DELETE statement targeting the lists table
    $sql = "DELETE FROM lists WHERE list_id IN ($placeholders)";
    ...
    echo json_encode([
        'success' => true,
        'message' => "Successfully deleted {$affected_rows} tag list(s)."
    ]);
} catch (PDOException $e) {
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    error_log("Database Error in ajax_delete_tag_lists.php: " . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'Database error during deletion.']);
} catch (Exception $e) {
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    error_log("General Error in ajax_delete_tag_lists.php: " . $e->getMessage());
